...
KeyType B_PLUS_TREE_INTERNAL_PAGE_TYPE::KeyAt(int index) const {
  ...
  assert(index >= 0 && index < GetSize());    //Determine whether the index is legitimate
  return array[index].first;    //Return the key value
}
...
ValueType B_PLUS_TREE_INTERNAL_PAGE_TYPE::ValueAt(int index) const {
  assert(index >= 0 && index < GetSize());    //Determine whether the index is legitimate
  return array[index].second;   //return value
}
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::MoveHalfTo(
    ...
    BufferPoolManager *buffer_pool_manager) {
      ...
      for(int i = copyIndex; i < n; i++){
        ...
        Page *childPage = buffer_pool_manager->FetchPage(array[i].second);
